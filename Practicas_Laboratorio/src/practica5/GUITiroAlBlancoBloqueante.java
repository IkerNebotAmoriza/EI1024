...
// ============================================================================
public class GUITiroAlBlancoBloqueante {
    // ============================================================================
    ...
    CanvasCampoTiroBloqueante cnvCampoTiro;
    JTextField         txfVelocidadInicial;
    JTextField         txfAnguloInicial;
    JButton            btnDispara;
    LinkedBlockingQueue<NuevoDisparo> ZonaDeIntercambio;
    MiHebraCalculadoraBloqueante h;
    NuevoDisparo ndisp;

    // --------------------------------------------------------------------------
    public static void main( String args[] ) {
        GUITiroAlBlancoBloqueante gui = new GUITiroAlBlancoBloqueante();
        gui.go();
    }
    ...
    public void go() {
        SwingUtilities.invokeLater(new Runnable(){
            public void run(){
                ...
                container = new JFrame( "GUI Tiro Al Blanco Bloqueante" );
                ...
                cnvCampoTiro = new CanvasCampoTiroBloqueante();
                jpanel.add( "Center", cnvCampoTiro );

                // Crea la zona de intercambio y la hebra, y arranca la hebra
                ZonaDeIntercambio = new LinkedBlockingQueue<NuevoDisparo>();
                h = new MiHebraCalculadoraBloqueante(cnvCampoTiro, txfMensajes, ZonaDeIntercambio);
                h.start();
                    ...
                    public void actionPerformed( ActionEvent e ) {
                        ...
                        try {
                            ...
                            if( ( 0.0 < angulo )&&( angulo < 90 )&&( velocidad > 0 ) ) {
                                ...
                                // Dibuja el proyectil.
                                ndisp = new NuevoDisparo(velocidad, angulo);
                                ZonaDeIntercambio.add(ndisp);
...
// ============================================================================
class CanvasCampoTiroBloqueante extends Canvas {
    // ============================================================================
...
class ProyectilBloqueante {
    ...
    // --------------------------------------------------------------------------
    ProyectilBloqueante( double velocidadInicial,  double anguloInicial ) {
    ...
    // --------------------------------------------------------------------------
    public void dibujaProyectil( final CanvasCampoTiroBloqueante cnvCampoTiro ) {
        ...
        if( ( this.intPosX != this.intPosXOld )||
            ...
            final int copiaPosXOld = intPosXOld;
            final int copiaPosYOld = intPosYOld;
            final int copiaPosX = intPosX;
            final int copiaPosY = intPosY;

            SwingUtilities.invokeLater(new Runnable() {
                @Override
                public void run() {
                    // Borra la posicion anterior.
                    cnvCampoTiro.borraProyectil( copiaPosXOld, copiaPosYOld );

                    // Dibuja la nueva posicion del proyectil.
                    cnvCampoTiro.dibujaProyectil( copiaPosX, copiaPosY );
                }
            });
        }
    }
